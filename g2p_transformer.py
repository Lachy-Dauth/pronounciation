...
class G2PTransformer(nn.Module):
    def __init__(self, char_vocab_size: int, phone_vocab_size: int, 
        ...
        # Output projection
        self.output_projection = nn.Linear(d_model, phone_vocab_size)
    ...
    def forward(self, src, tgt, src_padding_mask=None, tgt_padding_mask=None):
        ...
        src_emb = self.char_embedding(src) * math.sqrt(self.d_model)  # (batch, seq, d_model)
        tgt_emb = self.phone_embedding(tgt) * math.sqrt(self.d_model)
...
def inference(model, dataset, word: str, device, max_len=32):
    """Generate phonemes for a given word"""
    model.eval()
    
    # Convert word to indices
    char_indices = [dataset.char_to_idx.get(c.lower(), dataset.char_to_idx['<unk>']) 
                   for c in word]
    char_indices = [dataset.char_to_idx['<sos>']] + char_indices + [dataset.char_to_idx['<eos>']]
    char_indices = dataset._pad_sequence(char_indices, max_len, 0)
    
    src = torch.tensor(char_indices).unsqueeze(0).to(device)  # (1, seq_len)
    
    # Start with SOS token
    tgt = torch.tensor([[dataset.phone_to_idx['<sos>']]]).to(device)
    ...
    with torch.no_grad():
        for _ in range(max_len - 1):
            ...
            if next_token == dataset.phone_to_idx['<eos>']:
                break
            
            tgt = torch.cat([tgt, torch.tensor([[next_token]]).to(device)], dim=1)
    
    # Convert back to phones
    phone_indices = tgt[0][1:].cpu().numpy()  # Skip SOS token
    phones = [dataset.idx_to_phone[idx] for idx in phone_indices 
